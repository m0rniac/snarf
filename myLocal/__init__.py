# [Corpus; Unofficial Microsoft API SYNTHESIS from Microsoft Edge].
import re
import asyncio
import aiofiles
import os, os.path
...
""" 
Default ShortName: ru-RU-DmitryNeural
"""

# [Corpus; Bytes to STR].
bytesToStr = Union[str, bytes]

# [Corpus; Main DIR].
snarf_dir = os.path.dirname(__file__)

# [Corpus; Replace text fuction].
def ireplace(old, repl, text):
    return re.sub('(?i)'+re.escape(old), lambda m: repl, text)
class SNARFError(Exception):
    pass

# [Corpus; List of voices].
_voices_list: List = []

# [Corpus; INIT].
class SNARF:
    # {subCorpus; Unofficial API}
    ...
    # {subCorpus; Voice Getter (Singular)}
    ...
    async def parse_multivoices(
        ...
        # {subCorpus; Replacing voices with tags in text}
...
<voice  name='{voiceName}'><prosody pitch='{pitch}Hz' rate ='{rate}%' volume='{volume}%'>{text_from_tag}</prosody></voice>
    ...
    @staticmethod
    def _build_request(headers: Dict[str, str], body: bytesToStr) -> bytes:
        s: str = (
    ...
    @staticmethod
    def _extract_response(response: bytesToStr) -> Tuple[dict, bytesToStr]:
        offset: int = 130
        headers: Any = {}
        body: bytesToStr = ""
    ...
    # {subCorpus; New Type Voices Getter}
    async def get_voices_list(self) -> List[Dict]:
        ...
        if len(_voices_list) > 0:
            return _voices_list
        voicesplusfilepath = os.path.join(snarf_dir, "voices_list.json")
        # voicesplusfilepath = os.path.join(SNARF_dir, "voices_list_plus.json")
        if os.path.isfile(voicesplusfilepath):
            try:
                ...
                _voices_list = {}
                sys.stderr.write(f"SNARF.get_voices_list: error reading {voicesplusfilepath}")
        if len(_voices_list) > 0:
            return _voices_list
        async with aiohttp.ClientSession(headers=self.headers) as session:
            sys.stdout.write("SNARF.get_voices_list: downloading voice list JSON file...")
            async with session.get(
                ...
                try:
                    ...
                    sys.stderr.write(f"SNARF.get_voices_list: error ssaving {voicesplusfilepath}")
                return _voices_list
    ...
    async def synthesize(self, text: str, filename_or_buffer: Any, multivoices:bool = True) -> int:
        ...
        for rpcount in range(1, rplimit + 1):
            try:
                ...
                return res
            except (aiohttp.ClientError, ssl.SSLError, ValueError, SNARFError) as e:
                import sys
                sys.stderr.write(
                    f"SNARF.synthesize: {sys.exc_info()[1]}, repeat #{rpcount}"
                )
                if rpcount == rplimit:
                    raise
